...
Поддержаны HTTP `?model=/models/km/model.glb`, автоматический поиск соседнего `metadata.json` и drag-and-drop GLB с JSON. IFC-поля выводятся через `textContent`. Интерфейс на русском: resize, progress, выбор, одиночная подсветка, кнопки `Изолировать`, `Скрыть`, `Отменить скрытие`, `Приблизить`, `Копировать GUID`, `Снять выбор`. Кнопка `Вся модель` объединяет Show all и fit: возвращает скрытые/изолированные элементы и общий вид модели по диагонали сверху.
